...
**镜像不包含任何引擎。** 创建第一个账户后，初始引导选择器会列出可用的引擎。选择其中一个后，它会从 npm 安装到持久化的 `/data/agent/tools` 前缀中，在镜像更新后依然保留，并可从 设置 › 系统 › 引擎 单独更新。安装需要能访问 `registry.npmjs.org`，每个引擎约需 300–500 MB 磁盘空间。

**服务商凭据需自行准备。** Anthropic、OpenAI、OpenRouter、Gemini、Bedrock 等服务商需要订阅登录（如 Claude Pro、ChatGPT）或 API 密钥。在 设置 › 服务商 中输入一次即可，所有引擎都从同一处读取——切换引擎无需重新输入凭据。

**哪些数据会保留。** 关键的是两个挂载点：`/data` 存放账户、已安装的引擎、设置与会话历史；`/workspace` 是你的仓库。在进行重大镜像更新前请备份 `/data`——Cody 的数据库、引擎二进制文件和各种状态都在其中。

**如何更新。** Cody 自身随容器镜像更新（拉取最新标签，源码安装则为 `npm run build && npm start`）。引擎从 设置 › 系统 › 引擎 单独更新，新版本会重启活动会话，确保不会继续运行旧代码。
...
- **在 UI 中安装与更新**：引导流程中的选择界面，以及 设置 → 系统 › 引擎 都可以按需安装引擎，并为每个引擎提供**更新**按钮，就在版本检查旁边。更新当前活动的引擎会重启正在运行的会话，确保不会有会话继续使用过期的二进制文件。
- **API 密钥只需填一次**：在 设置 → 提供商 中输入一次提供商密钥（Anthropic、OpenAI、OpenRouter、Gemini、Bedrock 等），Cody 会以环境变量的形式交给每个引擎，效果与在容器上设置环境变量相同，切换引擎时无需重新填写凭证。订阅登录也在同一处完成：Claude Pro/Max、Codex 的 ChatGPT、GitHub Copilot 等都通过引擎自身的登录流程无头运行，Cody 会显示登录 URL；如果浏览器无法访问容器，把授权码或最终重定向 URL 粘贴回来即可（使用设备码的提供商则输入设备码）。omp 的模型注册表也在同一处，位于各提供商的详情视图中。
...
- Smart 路由在确定模型后仍保留专用标记，与手动选择明确区分。Fast 以推理设置旁的图标呈现，不支持或请求被拒绝时会说明原因。模型管理位于“设置 → Models”。
- 发送按钮旁的用量弹窗显示 Codex 已储存的重置次数。管理员确认具体账户后可使用一次；余额会刷新，获得新的重置次数时也会通知。
- “设置 → Preferences”可将工具和后台活动设为 Compact / Full / Hidden，不影响对话正文或思考内容。工具预设也移到这里，仅影响新会话。
- 最多可上传10张图片，系统会统一压缩以符合传输限制；支持的运行中会话可接收带图的引导消息。子代理记录从最新行打开并跟随新输出，手动向上滚动后停止跟随。
...
- **Claude Code / Codex 会话**：每个会话启动一个长期运行的进程，通过 stdio 使用 [Agent Client Protocol](https://agentclientprotocol.com) 通信，并在服务器端转换为与 UI 渲染相同的事件流。ACP 是这里唯一具备真正审批通道的传输方式，因此这些引擎可以在轮次中途暂停并请求确认；中止会取消该轮次，恢复则使用引擎原生的会话 ID。代理自身的权限模式（Claude 的 Manual / Accept edits / Plan / Auto、Codex 的审批级别）可在输入框中模型选择器旁边的选择器里切换。
