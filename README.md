...
- **Pick models without surprise state**: Smart routing keeps a distinct marker after resolving its model; manual picks remain distinct. Fast is an icon beside reasoning, with unsupported or rejected requests explained. Model management stays in Settings → Models.
- **See plan limits accurately**: quota follows the selected model's reported bucket. The Send-adjacent popup shows banked Codex resets, lets an administrator use one after account-specific confirmation, refreshes balances, and notifies when more resets arrive.
- **Keep the transcript focused**: Settings → Preferences offers Compact, Full, or Hidden tool/background activity without changing conversation text or thinking. Agent tools presets also live in Preferences and apply to new sessions.
- **Steer with screenshots**: attach up to ten images, automatically compressed together to fit the transport budget; supported active sessions accept image-bearing steering. Subagent transcripts open at the latest line and follow new output until you scroll up.
