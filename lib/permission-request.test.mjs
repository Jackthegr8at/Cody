...
test("the agent's options survive in the order it sent them", () => {
  const options = readPermissionOptions(SAMPLE_OPTIONS);
...
test("two options sharing a kind are two options", () => {
  // An agent may map its session-scoped grant onto allow_always because ACP has no
...
test("a request with nothing clickable is not a request", () => {
  ...
  assert.equal(readPermissionRequest({ options: SAMPLE_OPTIONS }), null);
...
test("get_state hydration reads the same shapes and ignores junk", () => {
  const requests = readPermissionRequests([
    { requestId: "perm-1", toolCall: null, options: SAMPLE_OPTIONS },
    { requestId: "perm-1", toolCall: null, options: SAMPLE_OPTIONS }, // duplicate id
...
test("an unknown kind is carried through verbatim rather than translated", () => {
  const summary = describeToolCall({ title: "Do a thing", kind: "custom_routine" });
  assert.equal(summary.kind, "custom_routine");
