...
test("distill is off for everyone who has not asked for it", () => {
  assert.deepEqual(DEFAULT_DISTILL_PREFERENCES, { replies: "off", thinking: false });
...
test("a partial object keeps the untouched half at its default", () => {
  assert.deepEqual(normalizeDistillPreferences({ thinking: true }), { replies: "off", thinking: true });
  assert.deepEqual(normalizeDistillPreferences({ replies: "medium" }), { replies: "medium", thinking: false });
});
