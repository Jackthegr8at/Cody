...
test("selectWindowsForModel routes past an exhausted sibling to the account still serving", () => {
  ...
  assert.deepEqual(ranked.map((entry) => entry.state), ["serving", "limited"]);
...
test("rankProviderAccounts: a measured account outranks an unmeasured sibling, both ahead of a limited one", () => {
  ...
  assert.deepEqual(ranked.map((entry) => entry.state), ["serving", "standby", "limited"]);
});
