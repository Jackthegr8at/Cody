...
// No forge config on disk: every fixture resolves to the built-in GitHub host,
// so the check is the same here as on a machine that has never opened
// Settings › Code hosts.
process.env.PI_CODING_AGENT_DIR = mkdtempSync(join(tmpdir(), "cody-skill-updates-"));
...
test("returns a scoped error when the remote check fails", async () => {
  ...
  // The message names the status the host answered with, so a 503 does not
  // read as "no update".
