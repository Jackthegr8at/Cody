...
test("wrapped child model events update visible model, reasoning, and fallback state", () => {
  assert.deepEqual(
    parseSubagentProgressEvent({ id: "Scout", event: { type: "model_changed", model: "openai/gpt-test:high" } }),
    { resolvedModel: "openai/gpt-test", thinkingLevel: "high" },
  );
  assert.deepEqual(
    parseSubagentProgressEvent({ id: "Scout", event: { type: "thinking_level_changed", thinkingLevel: "low" } }),
    { thinkingLevel: "low" },
  );
  assert.deepEqual(
    parseSubagentProgressEvent({ id: "Scout", event: { type: "retry_fallback_applied", from: "openai/large", to: "openai/small:low" } }),
    { resolvedModel: "openai/small", resolvedModelIsFallback: true, thinkingLevel: "low" },
  );
  const activity = parseSubagentActivityEvent({ id: "Scout", event: { type: "retry_fallback_applied", from: "openai/large", to: "openai/small" } });
  assert.equal(activity?.kind, "retry_fallback_applied");
  assert.equal(activity?.from, "openai/large");
  assert.equal(activity?.to, "openai/small");
});
