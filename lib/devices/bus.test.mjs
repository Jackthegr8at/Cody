...
test("a request rides to the attached page and settles on its result", async () => {
  ...
  bridge.settle({ type: "result", id: sent[0].id, status: "ok", value: { opened: true } });
  assert.deepEqual(await pending, { opened: true });

  const failing = bridge.request("serial.write", "d1", { base64: "AA==" });
  bridge.settle({ type: "result", id: sent[1].id, status: "error", error: "Port is closed" });
...
test("both directions of a transfer are counted where the bytes actually pass", async () => {
  ...
  bridge.settle({ type: "result", id: sent[0].id, status: "ok", value: { base64: Buffer.from("OKAYbiscuit").toString("base64") } });
  await pending;
...
test("a quiet read carries its bounded deadline and remains distinct from cancellation", async () => {
  const bridge = new DeviceBridge();
  const sent = [];
  bridge.attach((frame) => sent.push(frame));
  const quiet = bridge.request("usb.transfer", "u1", { direction: "in", endpoint: 1, length: 64 }, 125);
  assert.equal(sent[0].timeoutMs, 125);
  bridge.settle({ type: "result", id: sent[0].id, status: "no-data" });
  assert.deepEqual(await quiet, { noData: true });

  const cancelled = bridge.request("usb.transfer", "u1", { direction: "in", endpoint: 1, length: 64 }, 125);
  bridge.settle({ type: "result", id: sent[1].id, status: "cancelled", reason: "user stopped the read" });
  await assert.rejects(cancelled, /cancelled: user stopped the read/);
});

test("an acknowledged operation keeps its durable id and emits validated live progress", async () => {
  const bridge = new DeviceBridge();
  const sent = [];
  bridge.attach((frame) => sent.push(frame));
  const started = bridge.startOperation({ protocol: "esp", action: "detect", deviceId: "d1" });
  const command = sent[0];
  assert.equal(command.type, "operation");
  bridge.settle({ type: "result", id: command.command.id, status: "ok" });
  const id = await started;
  assert.equal(id, command.command.operationId);

  const snapshot = {
    id,
    sessionId: "session-a",
    request: { protocol: "esp", action: "detect", deviceId: "d1" },
    state: "running",
    createdAt: 1,
    updatedAt: 2,
    output: [],
    events: [{ sequence: 1, at: 2, type: "progress", progress: { phase: "detecting", at: 2 } }],
  };
  const received = [];
  bridge.onOperation((current, event) => received.push({ current, event }));
  bridge.receiveOperationProgress({ type: "operation.progress", operationId: id, snapshot, event: snapshot.events[0] });
  assert.equal(bridge.operationStatus(id)?.state, "running");
  assert.equal(received.length, 1);
  bridge.receiveOperationProgress({ type: "operation.progress", operationId: id, snapshot, event: snapshot.events[0] });
  assert.equal(received.length, 1, "replayed progress must not duplicate transcript updates");
  const completed = { ...snapshot, state: "succeeded", result: { summary: "detected", verified: true } };
  bridge.receiveOperationResult({ type: "operation.result", operationId: id, snapshot: completed });
  assert.equal(bridge.operationStatus(id)?.state, "succeeded", "terminal state must win even inside the same millisecond");
  assert.equal(received.length, 2);
});
test("a replacement host revokes the former authority and rejects its pending request", async () => {
  const bridge = new DeviceBridge();
  const first = bridge.attach(() => {}, () => { revoked = true; });
  let revoked = false;
  const pending = bridge.request("serial.open", "d1", { baudRate: 115200 });
  ...
  await assert.rejects(pending, /authority was replaced/);
});