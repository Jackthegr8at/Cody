...
// The bug this guards: an open that claimed nothing left every endpoint
// transfer failing with a browser error naming neither interface nor fix,
// so the agent could not tell dead hardware from an unclaimed one. What a
// caller must be able to read off an open is which protocol the device
// speaks and which endpoints to use.
test("device_open reports what a USB open claimed and what it speaks", async () => {
  const bridge = fakeBridge({
    devices: [usbDevice()],
    requestImpl: () => ({
      configuration: 1,
  ...
  const text = await tool("device_open").handler({ device: "u1" }, { bridge });
  assert.match(text, /configuration 1/);
  assert.match(text, /interface 0 claimed/);
  assert.match(text, /ff\/42\/03 \(fastboot\)/);
  assert.match(text, /bulk OUT ep1 \(512 B\)/);
  assert.match(text, /bulk IN ep1 \(512 B\)/);
});

test("an interface Windows would not release is named with its reason", async () => {
  const bridge = fakeBridge({
    devices: [usbDevice()],
    requestImpl: () => ({
      configuration: 1,
  ...
  const text = await tool("device_open").handler({ device: "u1" }, { bridge });
  assert.match(text, /interface 1 NOT claimed/);
  assert.match(text, /ff\/42\/01 \(adb\)/);
  assert.match(text, /Unable to claim interface\./);
  // The WinUSB remedy is host-specific: this fake browser is not Windows.
  assert.doesNotMatch(text, /Zadig/);
});

test("a transfer onto an unclaimed interface says how to recover", async () => {
  const bridge = fakeBridge({
    devices: [usbDevice()],
    requestImpl: () => { throw new Error("The specified endpoint is not part of a claimed and selected alternate interface."); },
  });
  const text = await tool("usb_transfer").handler({ direction: "in", endpoint: 1, length: 64 }, { bridge });
  assert.match(text, /device_open/);
  assert.match(text, /fresh grant/);
});
