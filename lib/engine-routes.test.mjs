...
test("/api/info publishes the active engine's identity and capabilities", async () => {
  ...
  // nativeSettings is TRUE since pi grew a settings surface of its own: the
  // settings tables in its installed package's docs/settings.md, written back
  // to `<pi agent dir>/settings.json`. configEditor stays false — that flag is
  // Cody's HAND-BUILT editors for omp's config.yml, which pi never reads, and
  // pi is now the engine that proves the two are different questions.
  assert.equal(piBody.capabilities.nativeSettings, true);
  assert.equal(piBody.capabilities.configEditor, false);
});
