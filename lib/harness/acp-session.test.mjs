...
test("an empty turn explains itself instead of leaving a void", async () => {
  ...
  const fresh = emptyTurnMessage("Codex", "end_turn", "Run `codex login` in a Cody terminal.");
  assert.match(fresh, /Codex ended the turn without a reply/);
  assert.match(fresh, /codex login/, "carries the engine's own remedy");
  ...
  assert.doesNotMatch(emptyTurnMessage("SomeAgent", "end_turn"), /codex/i, "no engine leaks into the generic path");

  // Distinct stop reasons get their own sentence rather than the setup guess.
  assert.match(emptyTurnMessage("Codex", "refusal"), /declined/);
  assert.match(emptyTurnMessage("Codex", "max_tokens"), /output limit/);
  assert.match(emptyTurnMessage("Codex", "cancelled"), /stopped before replying/);
  assert.doesNotMatch(emptyTurnMessage("Codex", "refusal"), /setup/, "a refusal is not a config problem");
});
...
test("ACP sends inline images and steers the active turn without ending it early", async () => {
  await withSession({
    ACP_STUB_DELAY_MS: "300",
    ACP_STUB_IMAGE_CAPABLE: "1",
    ACP_STUB_STEERING_CAPABLE: "1",
    ACP_STUB_ECHO_PROMPTS: "1",
  }, async (session, events) => {
    const initial = await session.send({ type: "get_state" });
    assert.equal(initial.imageSupported, true);
    assert.equal(initial.steeringSupported, true);

    const image = { type: "image", mimeType: "image/png", data: "YWJj" };
    await session.send({ type: "prompt", message: "describe this", images: [image] });
    await waitForEvent(events, "message_start");

    const beforeSteerEnds = events.filter((event) => event.type === "agent_end").length;
    const steering = await session.send({ type: "steer", message: "focus on the corner", images: [image] });
    assert.deepEqual(steering, { outcome: "injected" });
    assert.equal((await session.send({ type: "get_state" })).isPromptRunning, true, "steering must not clear the original active turn");
    assert.equal(events.filter((event) => event.type === "agent_end").length, beforeSteerEnds, "steering itself is not a terminal event");

    await waitForEvent(events, "agent_end");
    const messages = await session.send({ type: "get_messages" });
    const reply = messages.messages.at(-1).content[0].text;
    assert.match(reply, /prompt-wire:.*"type":"image".*"data":"YWJj".*"mimeType":"image\/png"/);
    assert.match(reply, /steer-wire:.*focus on the corner.*"type":"image"/);
  });
});

test("ACP refuses image prompts and steering that the server did not advertise", async () => {
  await withSession({}, async (session, events) => {
    const state = await session.send({ type: "get_state" });
    assert.equal(state.imageSupported, false);
    assert.equal(state.steeringSupported, false);
    const image = { type: "image", mimeType: "image/png", data: "YWJj" };
    await assert.rejects(session.send({ type: "prompt", message: "describe this", images: [image] }), (error) => error?.code === "unsupported");
    await assert.rejects(session.send({ type: "steer", message: "interrupt" }), (error) => error?.code === "unsupported");
    assert.ok(!events.some((event) => event.type === "agent_start"), "an unsupported image must never launch a text-only turn");
  });
});
for (const [outcome, code] of [["failed", "steering_failed"], ["promptRequired", "session_idle"], ["startedNewTurn", "session_idle"]]) {
  test("ACP preserves the active turn when steering reports " + outcome, async () => {
    await withSession({
      ACP_STUB_DELAY_MS: "300",
      ACP_STUB_STEERING_CAPABLE: "1",
      ACP_STUB_STEERING_OUTCOME: outcome,
    }, async (session, events) => {
      await session.send({ type: "prompt", message: "work" });
      await waitForEvent(events, "message_start");
      await assert.rejects(session.send({ type: "steer", message: "interrupt" }), (error) => error?.code === code);
      assert.equal((await session.send({ type: "get_state" })).isPromptRunning, true, "a rejected steering response must not end the original turn");
      await waitForEvent(events, "agent_end");
    });
  });
}
...
// the session-model one from a live `session/new` against an ACP agent that publishes no configOptions, which
...
/** As a live legacy-shape `session/new` answered, trimmed to three models. */
...
test("ACP rejects compact before it can fabricate compaction lifecycle events", async () => {
  await withSession({}, async (session, events) => {
    await assert.rejects(
      session.send({ type: "compact" }),
      (error) => error?.code === "unsupported",
    );
    assert.deepEqual(events, [], "unsupported compact must not start a turn or publish compaction progress");
  });
});