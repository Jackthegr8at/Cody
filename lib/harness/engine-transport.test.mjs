 ...
 *
 * Cody has two: omp's `rpcUi` NDJSON dialect and ACP for the open standard.
 * A third, a process-per-turn fallback, existed until Claude Code and Codex
 * moved to ACP and left it with no users; it was deleted rather than kept as
 * an untested extension point. `transportOf` still recognises a "turn" session
 * so that reintroducing one shows up here as a mismatch instead of passing
 * quietly.
 *
 * An engine silently moved to the wrong transport — or left on the old one
 * during a migration — is not a loud failure. It is a chat window that looks
 * fine and cannot approve a tool call.
 *
...
test("an ACP engine can actually be launched: spec, binary name and version probe", () => {
  for (const [id, expected] of Object.entries(EXPECTED_TRANSPORT)) {
    ...
    // The version probe has to run the REAL entry point, not a generic
    // --version, in case the binary's optional features are not installed.
...
test("every engine declares the full capability set", () => {
  ...
  const flags = [
    ...
    "configEditor", "updates", "chatExtras", "fastMode", "advisor",
    "subagents",
  ];
  for (const adapter of listHarnesses()) {
