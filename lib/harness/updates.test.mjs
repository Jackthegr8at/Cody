...
test("isBeyondVerifiedMajor flags only a provable jump past the marker", async () => {
  ...
  assert.equal(isBeyondVerifiedMajor("19.0.0", "18.0.11"), true);
  assert.equal(isBeyondVerifiedMajor("18.4.2", "18.0.11"), false);
  assert.equal(isBeyondVerifiedMajor("17.4.2", "18.0.11"), false);
  // Unknown version or unmarked adapter: never warn.
  assert.equal(isBeyondVerifiedMajor(null, "18.0.11"), false);
  assert.equal(isBeyondVerifiedMajor("garbage", "18.0.11"), false);
...
test("each engine's spec resolves to its own registry's URL and version field", async () => {
  ...
  try {
    const cases = [
      ...
      ["@oh-my-pi/pi-coding-agent", "https://registry.npmjs.org/%40oh-my-pi%2Fpi-coding-agent/latest", "9.9.9-npm"],
      ["@mariozechner/pi-coding-agent", "https://registry.npmjs.org/%40mariozechner%2Fpi-coding-agent/latest", "9.9.9-npm"],
      ["@agentclientprotocol/claude-agent-acp", "https://registry.npmjs.org/%40agentclientprotocol%2Fclaude-agent-acp/latest", "9.9.9-npm"],
      ["@agentclientprotocol/codex-acp", "https://registry.npmjs.org/%40agentclientprotocol%2Fcodex-acp/latest", "9.9.9-npm"],
      // The two companion CLIs, which the check now asks about in their own right.
      ["@anthropic-ai/claude-code", "https://registry.npmjs.org/%40anthropic-ai%2Fclaude-code/latest", "9.9.9-npm"],
      ["@openai/codex", "https://registry.npmjs.org/%40openai%2Fcodex/latest", "9.9.9-npm"],
    ];
    for (const [spec, url, expected] of cases) {
      calls.length = 0;
      // force=true to ensure fresh lookup, not cached.
      assert.equal(await fetchLatestPackageVersion(spec, true), expected, spec);
