...
// by running THAT. ACP engines like Claude and Codex have this pattern: their
// bare --version reports success whether or not the ACP dependencies are
// installed — so verifying with a bare --version blesses an install whose
// every chat turn then dies.
test("a subcommand probe catches what a bare --version would bless", async () => {
  const bin = script("extras-missing", [
    ...
    '  exit 1',
    'fi',
    'echo "Some Agent v0.19.0 (2026.7.20)"',
    'echo "Python: 3.11.15"',
  ].join("\n"));
