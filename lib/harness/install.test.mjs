...
test("installSteps installs the companion package on every run, with per-package flags", async () => {
  ...
  // One package, one step. Split engines like Claude use npm; uv is retired.
  assert.deepEqual(
...
// the picker card, the engine list in System › Engines, the Info panel.
// Reporting the ACP adapter's version there is not a smaller truth, it is a
// different package's version.
