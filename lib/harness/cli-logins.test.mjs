 ...
 * Claude Code's and Codex's provider-login surfaces
 * (claude-login.ts, codex-login.ts): it spawns a CLI,
 ...
 * per test — rather than the real `claude`/`codex` binaries, so the
 ...
 * This file pins the shared mechanism those two modules all depend on.
 */
