...
test("a saved chain keeps its order, drops duplicates and survives a reread", () => {
  ...
  if (process.platform !== "win32") assert.equal(fs.statSync(file).mode & 0o777, 0o600, "the chain is written 0600");
...
test("the cache key is entry, block, kind and verbosity", () => {
  assert.equal(cache.distillCacheKey("e1", 2, "thinking", undefined), "e1:2:thinking:-");
  assert.equal(cache.distillCacheKey("e1", undefined, "reply", "low"), "e1:-:reply:low");
});

test("a stored summary comes back for its own key and no other", () => {
  const entry = { text: "short version", model: "a/one", at: 1_000 };
  cache.writeDistillCache("sess-1", cache.distillCacheKey("e1", 0, "reply", "low"), entry);
...
test("a streaming one-shot reports deltas as they arrive and the full text at the end", { skip: process.platform === "win32" && "POSIX executable fixture" }, async () => {
...
test("a binary that exits non-zero with no frames is an error value, never a throw", { skip: process.platform === "win32" && "POSIX executable fixture" }, async () => {
