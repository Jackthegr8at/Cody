...
const { SIDEBAR_CONTEXT_TOOLS, SESSION_NOT_FOUND } = await jiti.import("./sidebar-context-tools.ts");
...
const { canAccessSession, setSessionOwner } = await jiti.import("./auth/session-owners.ts");
...
test("read_workspace_file rejects symlink escape", async () => {
  await withWorkspace(async (cwd) => {
...
test("the five tool schemas stay well under the 900-token sidebar budget", () => {
