...
test("connected with an uncounted catalog is pending, not zero models", () => {
  const readiness = deriveReadiness({
    engines: ENGINE_READY,
    providers: { providers: [{ connected: true, modelCount: 0 }], pending: { counts: true } },
  });
  assert.equal(readiness.provider, true);
