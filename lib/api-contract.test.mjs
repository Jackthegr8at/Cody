...
test("settings groups runtime preferences and resource managers behind tabs", async () => {
  ...
  const updates = await readFile(new URL("../components/settings/SystemUpdates.tsx", import.meta.url), "utf8");
  // Update surfaces are consolidated into the System and Updates section.
  assert.match(settings, /SystemUpdates/);
  assert.match(updates, /updates\.runCommandHint/);
  assert.match(updates, /updates\.omp\.restart/);
