...
const OMP_IMPORT_ALLOWLIST = {
  ...
  "app/api/models/new/route.ts": "rpc-dialect utility RPC: the catalog diffed against the seen ledger (dispatches on the active harness; an ACP engine answers empty before any child is spawned)",
  "app/api/models/visibility/route.ts": "reads omp's config.yml registry flag to report whether omp's instance hide (enabledModels) is writable; only when omp is the active engine",
  ...
  "lib/prompt-asset-index.ts": "plugin-management cache stores prompt files for sidebar chat's thinking distillation without invoking the OMP CLI at request time",
  ...
  "app/api/providers/verify/route.ts": "drops the shared utility RPC child before re-reading the catalog, so a key saved a moment ago is the one checked (dispatches on the active harness; refuses a non-rpc engine)",
  "hooks/useSchemaIndex.ts": "evaluates the engine's ui.condition predicates (lib/omp/settings-conditions) once, so a Recommended card and its schema row hide together; the one component-facing module that reads lib/omp, so no settings component needs this list",
  ...
  "app/api/sidebar-chats/route.ts": "omp sidebar-session storage in cody-sidebar-chats/ beneath omp's session directory",
  ...
  "lib/forge/config.ts": "instance data dir via omp/paths (code hosts + tokens)",
  "lib/project-registry.ts": "instance data dir via omp/paths",
  "lib/auth/paths.ts": "instance data dir via omp/paths (accounts store)",
  "lib/model-catalog-seen.ts": "instance data dir via omp/paths",
  "lib/model-visibility.ts": "instance data dir via omp/paths",
  "lib/local-model-profile-store.ts": "instance data dir via omp/paths",
  "lib/local-model-profile-runtime.ts": "omp models.yml plus settings-overlay paths for an omp-only launch profile",
  "lib/local-model-routing.ts": "Cody-owned Local-only sidecars and omp launch overlays",
  "lib/distill/config.ts": "instance data dir via omp/paths (Distill's model chain and its per-session summary cache; the summaries are Cody's own derived state and never go near the engine's session files)",
  "lib/plan-keeper/config.ts": "instance data dir via omp/paths (the plan keeper's enabled flag and its per-session subtask/auto-mark overlay directory — Cody's own derived state, never the engine's session files)",
  "lib/openrouter/key.ts": "instance data dir via omp/paths, and reads omp's own auth_credentials store read-only: `omp /login openrouter` is where most users' key actually lives, and omp exposes no command that prints one, so the environment alone would report 'no key' for an account that is spending right now",

  // --- omp-only features behind their own surfaces ---
  "lib/model-catalog-full.ts": "omp utility RPC: the unrestricted catalog behind a PI_CONFIG_FILES overlay, omp's own --config mechanism (served only under requireEngine('omp'))",
  "lib/models-effective.ts": "rpc-dialect utility RPC: the effective sessionless catalog behind /api/models and the per-provider counts of /api/providers (dispatches on the active harness; utilityRpcLaunchFor refuses a non-rpc engine)",
  ...
  "lib/plan-keeper/keeper.ts": "reads omp's smol/tiny model roles for the cheap-model plan-keeper check, the same lib/omp/model-roles.ts rule lib/session-namer.ts follows for tiny",
  ...
  "lib/routing/role-binding.ts": "writes omp's modelRoles: usage-aware role binding is omp-only (no other engine has roles or fallback chains), and the write is what reaches subagent spawns",
  "lib/routing/route-memory.ts": "instance data dir via omp/paths (remembered provider blackouts, role and fallback-chain baselines)",
  "lib/routing/chain-binding.ts": "writes omp's retry.fallbackChains: omp's reactive chain walk dials a candidate without asking about quota, so spent providers are left out of the written chain; omp-gated by reconcile.ts",
  "lib/routing/reconcile.ts": "reads omp's retry.fallbackChains and writes task.agentModelOverrides; omp-gated by getHarness().id",
  "lib/routing/request.ts": "instance data dir via omp/paths, to find omp's user-level agent definitions for role-alias enforcement",
  "lib/model-pricing-overlay.ts": "writes omp's models.yml modelOverrides so omp itself prices a prepaid provider's models; omp-gated by the caller",
  "lib/model-price-fill.ts": "writes omp's models.yml modelOverrides with models.dev prices for models omp's bundled catalog does not list yet; omp-gated by its own entry point",
  "lib/model-presets/store.ts": "Cody-owned preset store: validates presets against omp's live role vocabulary and reads the base modelRoles for copy-from-base and Smart resolution; served only under requireEngine('omp')",
  "lib/model-presets/overlay.ts": "writes per-conversation preset overlays (omp config layers passed via PI_CONFIG_FILES) into the instance data dir",
  "app/api/model-presets/route.ts": "reads the base modelRoles so the preset editor can show what an unset role inherits; requireEngine('omp')",
  "lib/model-presets/research.ts": "spawns the installed omp binary as the web-research child, in an isolated agent dir built from omp's own agent-dir layout",
  "app/api/model-presets/research/route.ts": "reads the base modelRoles to brief the research child on what each preset inherits; requireEngine('omp')",
 ...
 * not. */
const PRIVATE_HARNESS_MODULES = new Set(["omp", "pi", "claude", "codex", "acp-session"]);
