...
test("a slow ack on a queued send yields pending without destroying the child", async () => {
  let disposed = 0;
  let resolveSend;
  const sendPromise = new Promise((resolve) => { resolveSend = resolve; });
  const wrapper = new AgentSessionWrapper(
    {
      ...
      sendCommand: async (command, timeoutMs) => {
        if (command.type === "prompt") {
          assert.equal(timeoutMs, undefined, "a queued send must never carry the idle ack-recycle timeout");
          return sendPromise;
        }
        return {};
      },
  ...
  try {
    // Simulate an already-running turn: the wrapper's own state, not the
    // caller's guess, is what must decide this is a queue.
    wrapper.promptRunning = true;

    const raced = await Promise.race([
      wrapper.send({ type: "prompt", message: "keep going", streamingBehavior: "followUp", clientMessageId: "slow-1" }).then(() => "settled"),
      new Promise((resolve) => setTimeout(resolve, 20)).then(() => "still-pending"),
    ]);
    assert.equal(raced, "still-pending", "a caller giving up after its own wait bound must see the send still in flight, not settled early");
    assert.equal(disposed, 0, "a slow ack on an active turn must never recycle the child");
    assert.equal(wrapper.isAlive(), true);

    resolveSend({});
    const result = await wrapper.send({ type: "prompt", message: "keep going", streamingBehavior: "followUp", clientMessageId: "slow-1" });
    assert.deepEqual(result, { delivery: "queued", clientMessageId: "slow-1" });
