...
const { AgentSessionWrapper, WebRpcError, buildEngineRpcLaunch, buildSessionSpawnArgs, guardHostToolResultFrame, launchWithLocalRouting } = await jiti.import("./rpc-manager.ts");
...
test("Local-only rejects an out-of-snapshot set_model before the RPC process mutates", async () => {
  ...
  try {
    ...
    const mixedLaunch = launchWithLocalRouting(undefined, "mixed-local-routing");
...
test("buildSessionSpawnArgs with kind: 'sidebar' includes all required flags", () => {
  ...
  assert.ok(args[systemPromptIdx + 1]?.includes("side panel"), "system prompt mentions side panel context");
  assert.ok(args[systemPromptIdx + 1]?.includes("cannot read or edit files"), "system prompt mentions file editing restriction");
});
