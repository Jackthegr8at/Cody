...
const { claudeDisplayMcpConfig, createDisplayMcpLaunch, displayMcpAcpServer } = await jiti.import("./display/engine-tools.ts");
...
test("display candidate ladder is best-first and always ends with the streamed floor", async () => {
  ...
  try {
    ...
    // A dead target earns no gateway rung — the gateway cannot render what
    // nobody serves. (lib/native-gateway.test.mjs covers the full
    // renderability matrix: auth redirects, 403s, frame-blocking headers.)
    assert.deepEqual((await resolveDisplayCandidates(`http://localhost:${dead}/app?q=1`, "auto")).map((candidate) => candidate.kind), ["stream"]);

    const live = http.createServer((_request, response) => { response.writeHead(200, { "content-type": "text/html" }); response.end("ok"); });
    await new Promise((resolve) => live.listen(0, "127.0.0.1", resolve));
    try {
      const ladder = await resolveDisplayCandidates(`http://localhost:${live.address().port}/app?q=1`, "native");
...
test("the display MCP launcher carries scoped display and to-do capabilities", () => {
  const launch = createDisplayMcpLaunch("session-a", "Claude Code");
  assert.equal(verifyDisplayCapability(launch.capability)?.sid, "session-a");
  assert.equal(launch.todoEndpoint, (process.env.CODY_INTERNAL_DISPLAY_ORIGIN ?? "").replace(/\/$/, "") + "/api/internal/todo");
  assert.equal(launch.engineLabel, "Claude Code");
  ...
  assert.equal(claude.mcpServers.cody_display.env.CODY_TODO_ENDPOINT, launch.todoEndpoint);
  assert.equal(claude.mcpServers.cody_display.env.CODY_ENGINE_LABEL, "Claude Code");

  const acp = displayMcpAcpServer("session-a", "Codex");
  assert.equal("type" in acp, false);
  assert.deepEqual(acp.env.find((entry) => entry.name === "CODY_ENGINE_LABEL"), { name: "CODY_ENGINE_LABEL", value: "Codex" });
  assert.equal(verifyDisplayCapability(acp.env.find((entry) => entry.name === "CODY_DISPLAY_CAPABILITY")?.value)?.sid, "session-a");
});
