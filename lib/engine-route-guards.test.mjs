...
// "No engine binaries installed" is the premise of this whole file. Pin every
// binary override before importing routes so host tools and user-level config
// cannot alter the failure boundary being exercised.
...
const providersVerifyRoute = await jiti.import("../app/api/providers/verify/route.ts");
...
const ACP_ENGINES = ["claude", "codex"];
...
test("provider sign-in lists through the adapter under every engine, never through omp", async () => {
  ...
  // pi the roster is pi's own OAuth providers, read through pi's own CLI,
...
test("the two settings routes are gated by the two different flags they answer to", async () => {
  ...
  // active engine declares (omp's TypeScript schema, pi's own docs/settings.md)
  // through the same panel — `nativeSettings`.
  //
  // Guard the schema route on configEditor and an engine keeps a settings tab
  // whose contents nothing can search; guard the values route on
  // nativeSettings and an engine gets a red banner over the whole dialog. Both
...
test("a PUT never falls through to another engine's config file", async () => {
  ...
  // an `if (someEngine) … else ompWriter(patch)` shape, so every engine without a
  ...
  // Codex, with no codex installed: same shape, its own words.
  selectEngine("codex");
  const codex = await json(await put({ "display.show_reasoning": true }));
  assert.equal(codex.status, 400);
  assert.match(String(codex.body.error), /codex/i);
});

test("plan quota belongs to the accounts, so any engine may read it once omp is installed", async () => {
  // Quota is spent by the ACCOUNT, whichever engine sends the request, so
  // the route no longer refuses a non-omp engine. Its one precondition is the
  // omp binary (the only reader), and none is installed here: the answer is
  // an unavailable VALUE naming omp, never a 4xx and never the engine.
  for (const id of ["codex", "claude", "omp"]) {
    selectEngine(id);
    const response = await usageRoute.GET(new Request("http://cody.test/api/usage", { headers: AUTH }));
    const body = await response.json();
    assert.equal(response.status, 200, `${id}: the quota meter is never an error surface`);
    assert.equal(body.available, false);
    assert.deepEqual(body.accounts, []);
    assert.match(body.reason, /omp is not installed/, `${id}: the reason names the missing reader, not the engine`);
  }
});
...
test("marking the catalog seen is an administrator's act, and the ledger is per engine", async () => {
  ...
  assert.equal(written.body.engine?.id, "codex");
  ...
  assert.equal(other.body.engine?.id, "pi");
  assert.deepEqual(other.body.seenKeys, []);
  assert.equal(other.body.seenAt, null);
});

test("the cached-only new-models peek never spawns an engine: a cold cache answers pending", async () => {
  // The settings rail, the composer footer and the post-install toast all
  // read `?cached=1`. No omp binary is installed here, so a route that tried
  // to fill the cache would fail with "omp binary not found" — and the
  // absence of that modelError is the proof nothing was spawned.
  selectEngine("omp");
  const started = Date.now();
  const { status, body } = await json(await newModelsRoute.GET(new Request("http://cody.test/api/models/new?cached=1")));
  assert.equal(status, 200);
  assert.equal(body.pending, true, "a cold cache is reported, not filled");
  assert.deepEqual(body.newModels, []);
  assert.equal(body.total, 0);
  assert.equal(body.catalogSource, "global");
  assert.equal(body.modelError, undefined, "no child was started, so nothing failed");
  assert.ok(Date.now() - started < 1_000, "answered from memory, not from a process");

  // The same peek under pi reads pi's effective-catalog entry, equally cold.
  selectEngine("pi");
  const pi = await json(await newModelsRoute.GET(new Request("http://cody.test/api/models/new?cached=1")));
  assert.equal(pi.body.pending, true);
  assert.equal(pi.body.modelError, undefined);
});

test("model visibility answers under every engine, and omp refuses a Cody-file instance hide", async () => {
  for (const id of ["omp", "pi", ...ACP_ENGINES]) {
    selectEngine(id);
    const { status, body } = await json(await visibilityRoute.GET(new Request("http://cody.test/api/models/visibility", { headers: AUTH })));
    assert.equal(status, 200, `${id}: visibility is a read every engine serves`);
    assert.equal(body.engine?.id, id);
    assert.equal(body.instanceSource, id === "omp" ? "enabledModels" : "cody");
  }
  // omp's instance-wide hide is its own `enabledModels`, written through
  // /api/omp-settings; a PUT of `instanceHidden` there must refuse with the
  // code the UI hides on rather than start a second dialect of the setting.
  selectEngine("omp");
  const refused = await json(await visibilityRoute.PUT(new Request("http://cody.test/api/models/visibility", {
    method: "PUT",
    headers: { "Content-Type": "application/json", ...AUTH },
    body: JSON.stringify({ instanceHidden: ["acme/alpha"] }),
  })));
  assert.equal(refused.status, 400);
  assert.equal(refused.body.code, "unsupported");
});

test("the Providers hub answers under every engine, and Verify refuses where there is no catalog to read", async () => {
  // GET /api/providers composes sign-ins, keys and per-provider model counts.
  // With no binary anywhere it must still answer 200 for all five: an ACP
  // engine has null counts by design (its models live in the session), and
  // an rpc-dialect engine whose binary is missing puts the failure on the
  // rows as `reason` instead of answering 500 — the hub is not an error
  // surface for a fact about the engine.
  for (const id of ["omp", "pi", ...ACP_ENGINES]) {
    selectEngine(id);
    const { status, body } = await json(await providersRoute.GET(new Request("http://cody.test/api/providers", { headers: AUTH })));
    assert.equal(status, 200, `${id}: the hub must answer (got ${JSON.stringify(body).slice(0, 120)})`);
    assert.equal(body.engine?.id, id, `${id}: the hub is stamped with its engine`);
    assert.ok(Array.isArray(body.providers) && body.providers.length > 0, `${id}: the key catalogue alone gives rows`);
    if (ACP_ENGINES.includes(id)) {
      assert.ok(body.providers.every((row) => row.modelCount === null), `${id}: an ACP engine has no sessionless count`);
      assert.equal(body.canVerify, false);
    }
  }
  // Verify reads the sessionless catalog; an engine without one refuses with
  // the code the UI hides on.
  for (const id of ACP_ENGINES) {
    selectEngine(id);
    const { status, body } = await json(await providersVerifyRoute.POST(new Request("http://cody.test/api/providers/verify", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...AUTH },
      body: JSON.stringify({ providerId: "anthropic" }),
    })));
    assert.equal(status, 400, `${id}: verify answered ${status}`);
    assert.equal(body.code, "unsupported");
  }
});

test("the schema route always carries secretsSet, and a secret leaf's value never leaves the server", async () => {
  // Shape first: with no binary the schema is null and the list is empty —
  // the client reads `secretsSet` unconditionally, so it must exist even
  // when there is nothing to redact.
  for (const id of ["omp", "pi"]) {
    selectEngine(id);
    const { status, body } = await json(await ompSettingsSchemaRoute.GET());
    assert.equal(status, 200);
    assert.deepEqual(body.secretsSet, [], `${id}: secretsSet is an (empty) list beside a null schema`);
  }

  // Now a pi whose docs/settings.md declares an apiKey beside an ordinary
  // setting, and a settings.json holding a value for both. The stub package
  // is the shape an npm install leaves: docs/settings.md sits beside the
  // binary's package.json, so the adapter's own derivation runs unchanged.
  const { invalidateEngineBinCache } = await jiti.import("./harness/engine-bin.ts");
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "cody-pi-secret-"));
  const bin = path.join(root, "bin");
  fs.mkdirSync(bin, { recursive: true });
  fs.mkdirSync(path.join(root, "docs"), { recursive: true });
  fs.writeFileSync(
    path.join(root, "package.json"),
    JSON.stringify({ name: "@mariozechner/pi-coding-agent", version: "1.2.3" }),
  );
  fs.writeFileSync(
    ...
    [
      ...
      "| --- | --- | --- | --- |",
      '| auxiliary.apiKey | string | `""` | API key for the auxiliary vision model. |',
      "",
      "## Agent",
      "",
      "| Setting | Type | Default | Description |",
      "| --- | --- | --- | --- |",
      "| agent.maxTurns | number | `90` | Maximum turns per session. |",
      "",
    ].join("\n"),
  );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ auxiliary: { apiKey: "sk-live-do-not-echo" }, agent: { maxTurns: 50 } }),
  );

  const previousBin = process.env.CODY_PI_BIN;
  process.env.CODY_PI_BIN = path.join(bin, "pi");
  invalidateEngineBinCache("pi");
  try {
    selectEngine("pi");
    const response = await ompSettingsSchemaRoute.GET();
    const text = await response.text();
    const body = JSON.parse(text);
    assert.equal(response.status, 200);
    const apiKey = body.schema.settings.find((setting) => setting.key === "auxiliary.apiKey");
    assert.equal(apiKey.secret, true, "the apiKey leaf is declared secret");
    assert.equal(body.schema.settings.find((setting) => setting.key === "agent.maxTurns").secret, undefined);
    assert.equal(body.values["agent.maxTurns"], 50, "ordinary values still flow");
    assert.equal("auxiliary.apiKey" in body.values, false, "the secret's value is withheld");
    assert.deepEqual(body.secretsSet, ["auxiliary.apiKey"], "…but the fact that one is set is reported");
    assert.doesNotMatch(text, /sk-live-do-not-echo/, "the value appears nowhere in the response");

    // A write echoes values back; those are redacted by the same rule.
    const put = await ompSettingsSchemaRoute.PUT(new Request("http://cody.test/api/omp-settings/schema", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ patch: { "auxiliary.apiKey": "sk-new-value" } }),
    }));
    const putText = await put.text();
    const putBody = JSON.parse(putText);
    assert.equal(put.status, 200);
    assert.deepEqual(putBody.written, ["auxiliary.apiKey"]);
    assert.equal("auxiliary.apiKey" in putBody.values, false);
    assert.deepEqual(putBody.secretsSet, ["auxiliary.apiKey"]);
    assert.doesNotMatch(putText, /sk-new-value|sk-live-do-not-echo/);
  } finally {
    if (previousBin === undefined) delete process.env.CODY_PI_BIN;
    else process.env.CODY_PI_BIN = previousBin;
    invalidateEngineBinCache("pi");
