...
process.env.CODY_HERMES_BIN = path.join(agentDir, "no-such-hermes");
process.env.HERMES_HOME = path.join(agentDir, "hermes-home");
...
test("the schema route always carries secretsSet, and a secret leaf's value never leaves the server", async () => {
  ...
  // Now a Hermes whose DEFAULT_CONFIG declares an api_key beside an ordinary
  // setting, and a config.yaml holding a value for both. The stub venv is the
  // shape `uv tool install` leaves: the python that can import hermes_cli
  // sits beside the binary, so the adapter's own derivation runs unchanged.
  const { invalidateEngineBinCache } = await jiti.import("./harness/engine-bin.ts");
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "cody-hermes-secret-"));
  const bin = path.join(root, "bin");
  fs.mkdirSync(bin, { recursive: true });
  fs.writeFileSync(path.join(root, "pyvenv.cfg"), "home = /usr\n");
  fs.writeFileSync(path.join(bin, "hermes"), "#!/bin/sh\nexit 0\n", { mode: 0o755 });
  const defaults = JSON.stringify({ auxiliary: { vision: { api_key: "", provider: "auto" } }, agent: { max_turns: 90 } });
  fs.writeFileSync(path.join(bin, "python"), `#!/bin/sh\ncat <<'JSON'\n${defaults}\nJSON\n`, { mode: 0o755 });
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "cody-hermes-secret-home-"));
  fs.writeFileSync(path.join(home, "config.yaml"), "auxiliary:\n  vision:\n    api_key: sk-live-do-not-echo\nagent:\n  max_turns: 50\n");

  const previousBin = process.env.CODY_HERMES_BIN;
  const previousHome = process.env.HERMES_HOME;
  process.env.CODY_HERMES_BIN = path.join(bin, "hermes");
  process.env.HERMES_HOME = home;
  invalidateEngineBinCache("hermes");
  try {
    selectEngine("hermes");
    ...
    const apiKey = body.schema.settings.find((setting) => setting.key === "auxiliary.vision.api_key");
    assert.equal(apiKey.secret, true, "the api_key leaf is declared secret");
    assert.equal(body.schema.settings.find((setting) => setting.key === "agent.max_turns").secret, undefined);
    assert.equal(body.values["agent.max_turns"], 50, "ordinary values still flow");
    assert.equal("auxiliary.vision.api_key" in body.values, false, "the secret's value is withheld");
    assert.deepEqual(body.secretsSet, ["auxiliary.vision.api_key"], "…but the fact that one is set is reported");
    ...
    const put = await ompSettingsSchemaRoute.PUT(new Request("http://cody.test/api/omp-settings/schema", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ patch: { "auxiliary.vision.api_key": "sk-new-value" } }),
    }));
    const putText = await put.text();
    const putBody = JSON.parse(putText);
    assert.equal(put.status, 200);
    assert.deepEqual(putBody.written, ["auxiliary.vision.api_key"]);
    assert.equal("auxiliary.vision.api_key" in putBody.values, false);
    assert.deepEqual(putBody.secretsSet, ["auxiliary.vision.api_key"]);
    assert.doesNotMatch(putText, /sk-new-value|sk-live-do-not-echo/);
  } finally {
    if (previousBin === undefined) delete process.env.CODY_HERMES_BIN;
    else process.env.CODY_HERMES_BIN = previousBin;
