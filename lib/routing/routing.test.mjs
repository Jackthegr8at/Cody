...
test("observation never writes to the engine's config; binding is opt-in", async (t) => {
  ...
  copyFileSync(new URL("./fixtures/routing-config.yml", import.meta.url).pathname, config);
  ...
  const exhausted = snapshot([account({
    ...
    id: "codex",
    credentialId: 1,
    windows: [window({ utilization: 100, state: "exhausted", resetsAt: "2026-09-20T03:15:02.000Z" })],
  })]);
...
test("a prepaid balance blacks out only on a successful read of zero", () => {
