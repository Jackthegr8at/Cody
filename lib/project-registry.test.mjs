...
async function withAgentDir(t) {
  const agentDir = mkdtempSync(join(tmpdir(), "cody-registry-test-"));
  const previous = {
...
test("validateProjectPath rejects empty, missing, and non-directory paths", (t) => {
  const root = mkdtempSync(join(tmpdir(), "cody-path-test-"));
...
test("worktree paths canonicalize to the main repo root when registered", async (t) => {
  ...
  const root = mkdtempSync(join(tmpdir(), "cody-registry-worktree-"));
  const repo = join(root, "repo");
  const worktreeBase = `${repo}-worktrees`;
  try {
    execFileSync("git", ["init", repo], { stdio: "ignore" });
    execFileSync("git", ["-C", repo, "config", "user.email", "cody@example.invalid"], { stdio: "ignore" });
    execFileSync("git", ["-C", repo, "config", "user.name", "cody test"], { stdio: "ignore" });
    ...
    assert.equal(project.projectRoot, repo);
    ...
    assert.equal(registered.projects[0].path, repo);

    // Sessions discovered from the worktree carry the main repo as their
    // projectRoot, so they merge into the same project.
    const merged = mergeProjects(registered, [project.projectRoot, repo]);
    assert.deepEqual(merged.map((p) => p.path), [repo]);
  } finally {
...
test("resolveProject returns canonical on-disk casing for plain directories", (t) => {
  const root = mkdtempSync(join(tmpdir(), "cody-canonical-"));
  t.after(() => rmSync(root, { recursive: true, force: true }));
