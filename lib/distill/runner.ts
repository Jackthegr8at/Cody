...
import { buildDistillPrompt, normalizeThinkingSummary, type DistillKind, type DistillVerbosity } from "./prompts";

/**
 * Running one distill: which engine can, which models to try, and how many at
 * once.
 *
 * Three rules hold this together, and all three exist so the feature cannot
 * break when the engine underneath it changes.
 *
 *  1. FALL THROUGH, never fail hard. A spawn failure, a non-zero exit, a model
 *     the engine no longer knows, a timeout and an empty answer are ONE case:
 *     try the next selector. After the last one, try the engine's own default
 *     with no --model at all, which is the only attempt that cannot be
 *     invalidated by the catalog changing. Only then is the distill reported
 *     as failed — and a failed distill leaves the original text exactly as it
 *     was, so the worst outcome is that a summary does not appear.
...
export const THINKING_TIMEOUT_MS = 20_000;
...
export async function runDistillChain(options: DistillChainOptions): Promise<DistillChainResult> {
  const { systemPrompt, prompt } = buildDistillPrompt(options.kind, options.verbosity, options.text);
...
function acquireSlot(key: string | null): Promise<void> {
  ...
  if (waiting.length >= MAX_QUEUED_DISTILLS) {
    waiting.shift()?.reject(new Error("too many summaries are already queued"));
  }
