...
test("peekCatalogCache reads a stored entry, fresh or expired, and never loads", async () => {
  const { loadCatalogWithCache, peekCatalogCache } = await import("./models-cache.ts");
  invalidateModelsCache();
  assert.equal(peekCatalogCache("full:peek"), undefined, "a cold cache is simply undefined");
  await loadCatalogWithCache("full:peek", async () => ["a", "b"], { ttlMs: 1 });
  assert.deepEqual(peekCatalogCache("full:peek"), ["a", "b"]);
  await new Promise((resolve) => setTimeout(resolve, 5));
  // Expired is still the current answer for the stale-while-revalidate
  // serving path, so the peek returns it rather than pretending it is gone.
  assert.deepEqual(peekCatalogCache("full:peek"), ["a", "b"], "an expired entry is still peekable");
  invalidateModelsCache();
  assert.equal(peekCatalogCache("full:peek"), undefined, "invalidation clears it");
});
