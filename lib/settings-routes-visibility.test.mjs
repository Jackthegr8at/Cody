...
const ENGINES = ["omp", "pi", "claude", "codex"];
...
test("an administrator's instance hide lands in Cody's file on a non-omp engine and is refused on omp", async () => {
  selectEngine("codex");
  const written = await json(await put(ADMIN, { instanceHidden: ["codex-model/two", "codex-model/one"] }));
  assert.equal(written.status, 200);
  assert.deepEqual(written.body.instanceHidden, ["codex-model/one", "codex-model/two"]);
  ...
  assert.deepEqual(memberView.body.instanceHidden, ["codex-model/one", "codex-model/two"]);
  const file = JSON.parse(fs.readFileSync(path.join(agentDir, "cody-model-visibility.json"), "utf8"));
  assert.deepEqual(file.engines.codex, { hidden: ["codex-model/one", "codex-model/two"] });
...
test("lists are per engine", async () => {
  ...
  assert.deepEqual(codex.body.hidden, [], "nothing hidden under codex per-user");
  assert.deepEqual(codex.body.instanceHidden, ["codex-model/one", "codex-model/two"], "codex instance hide set by admin in previous test");
});
