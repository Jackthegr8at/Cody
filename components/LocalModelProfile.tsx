...
function ProfileSelect({ value, label, disabled, onChange }: { value: PromptProfileOverride; label: string; disabled?: boolean; onChange: (value: PromptProfileOverride) => void }) {
  const { t } = useI18n();
  const options: SelectOption<PromptProfileOverride>[] = [{ value: "auto", label: promptProfileLabel(t, "auto") }, ...PROFILE_IDS.map((profile) => ({ value: profile, label: promptProfileLabel(t, profile) }))];
  return <Select value={value} onChange={onChange} options={options} size="sm" width="128px" aria-label={label} disabled={disabled} />;
}
