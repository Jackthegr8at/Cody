...
/**
 * The Account settings panel: the signed-in profile (name, picture,
 * password, sign out), the access tokens, and, for administrators, the
 * server's account roster and self-service-signup status. Follows the
 * settings-panel convention of NativeSetting cards so search highlighting
 * and theming come for free.
 *
 * The one destructive action — deleting another account — lives in the
 * Danger zone at the foot of the panel, behind a ConfirmDialog; a password
 * reset takes its value through a PromptDialog. The agent engine card that
 * used to sit here is System › Engines now; a pointer says so.
 */
...
/** A block that answers the search highlight the way a NativeSetting card
 * does: scrolls into view and wears the outline while its id matches. */
function SearchBlock({ id, children }: { id: string; children: React.ReactNode }) {
  ...
  return (
    <div ref={ref} data-search-id={id} style={{ display: "flex", flexDirection: "column", gap: 10, borderRadius: "var(--radius-card)", transition: "box-shadow var(--dur-fast)", ...(highlighted ? { boxShadow: "0 0 0 2px var(--accent)" } : {}) }}>
      {children}
    </div>
  );
}

export function AccountSettings({ isMobile, onOpenSystem }: { isMobile: boolean; onOpenSystem?: () => void }) {
  const { t } = useI18n();
  ...
  /** The account whose password is being reset (PromptDialog open). */
  const [resetTarget, setResetTarget] = useState<PublicUser | null>(null);
  /** The Danger zone's chosen account and whether its ConfirmDialog is open. */
  ...
  const resetPassword = (target: PublicUser, next: string) => {
    setResetTarget(null);
    setRosterBusy(target.id);
    requestJson<{ user: PublicUser }>(`/api/accounts/users/${target.id}`, {
      method: "PATCH",
      ...
      .then(() => {
        setRosterError(null);
        toast.success(`Password reset for @${target.username}`, "Their other devices are signed out and their access tokens are revoked.");
      })
      .catch((error: unknown) => setRosterError(error instanceof Error ? error.message : String(error)))
      .finally(() => setRosterBusy(null));
  };
  ...
  const removeUser = (target: PublicUser) => {
    ...
    requestJson<{ success: boolean }>(`/api/accounts/users/${target.id}`, { method: "DELETE" })
      .then(() => {
        setRosterError(null);
        setDeleteChoice("");
        toast.success(`Deleted @${target.username}`, "Their sessions remain on disk without an owner.");
        reloadRoster();
      })
      .catch((error: unknown) => setRosterError(error instanceof Error ? error.message : String(error)))
      .finally(() => setRosterBusy(null));
  ...
  // Docker-managed accounts and your own cannot be deleted here.
  const deletable = (users ?? []).filter((user) => user.id !== me.id && !user.envManaged);
  const deleteTarget = deletable.find((user) => user.id === deleteChoice) ?? null;
  ...
  return (
    <div role="tabpanel" id="settings-panel-accounts" aria-labelledby="settings-tab-accounts" style={{ padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <NativeSetting
        ...
        control={me.envManaged ? undefined : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 420 }}>
            <input type="password" placeholder="Current password" aria-label="Current password" autoComplete="current-password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} style={nativeInputStyle} />
            <input type="password" placeholder={`New password (at least ${MIN_PASSWORD_LENGTH} characters)`} aria-label="New password" autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} style={nativeInputStyle} />
            <input type="password" placeholder="Repeat new password" aria-label="Repeat new password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} style={nativeInputStyle} />
      ...
      {isAdmin && (
        <>
          <SearchBlock id="accounts-on-this-server">
          ...
          {/* The engine card that lived here is System › Engines now. */}
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", padding: "10px 14px", border: "1px dashed var(--border)", borderRadius: "var(--radius-card)" }}>
            <Cpu size={14} aria-hidden style={{ color: "var(--text-dim)", flexShrink: 0 }} />
            <span style={{ flex: "1 1 220px", fontSize: 11.5, color: "var(--text-muted)", lineHeight: 1.5 }}>{t("updates.engines.settingsPointer")}</span>
            {onOpenSystem && (
              <button type="button" onClick={onOpenSystem} style={smallButtonStyle}>Open System</button>
            )}
          </div>

          <SearchBlock id="account-danger-zone">
            <DangerZone
              rows={[{
                ...
                action: (
                  <>
                    ...
                    <button
                      ...
                      disabled={!deleteTarget || rosterBusy !== null}
                      style={{ ...dangerButtonStyle, opacity: !deleteTarget || rosterBusy !== null ? 0.6 : 1 }}
                    >
          ...
          <PromptDialog
            ...
            label="New password"
            description="Their other devices are signed out and their access tokens are revoked. Share the new password with them out of band."
            placeholder={`At least ${MIN_PASSWORD_LENGTH} characters`}
            secret
            confirmLabel="Reset password"
            validate={(value) => (value.length < MIN_PASSWORD_LENGTH ? `Use at least ${MIN_PASSWORD_LENGTH} characters` : null)}
            busy={resetTarget !== null && rosterBusy === resetTarget.id}
            onSubmit={(value) => { if (resetTarget) resetPassword(resetTarget, value); }}
          ...
          <ConfirmDialog
            ...
            title={deleteTarget ? `Delete @${deleteTarget.username}?` : "Delete account"}
            description={deleteTarget ? `${deleteTarget.fullName} loses access immediately. Their sessions remain on disk but lose their owner; their access tokens stop working.` : ""}
            confirmLabel="Delete account"
            danger
            busy={deleteTarget !== null && rosterBusy === deleteTarget.id}
            onConfirm={() => { if (deleteTarget) removeUser(deleteTarget); }}
          />
        </>
      )}
    </div>
