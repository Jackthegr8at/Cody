 ...
 * per roster engine (System), one `mcp-<name>` per inventory server
 * (Extensions), and the Models hub's Distill row, which exists only when
 * `/api/distill/config` says this engine can distill. Each hub exports its
 * hook; this component is the one place that calls them, renders nothing,
 * and hands the union up to the shell.
 *
...
import { useModelsSearchEntries } from "./panels/ModelsPanel";
...
export function SearchSources({ onChange }: { onChange: (entries: readonly SearchEntry[]) => void }) {
  ...
  const models = useModelsSearchEntries();
  const entries = useMemo(() => [...system, ...extensions, ...models], [system, extensions, models]);
