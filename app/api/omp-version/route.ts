 ...
 *
 * It used to probe omp unconditionally, so a non-omp engine instance would
 * report omp's 18.0.1 as its engine version. The engine identity now rides
 * along, because a bare version string is exactly what let the caller assume
 * whose it was.
 */
